...
export default function ParametresClient({ twoFactorEnabled: initial2FA }: ParametresClientProps) {
  const [twoFactorEnabled, setTwoFactorEnabled] = useState(initial2FA);
  const [isToggling, setIsToggling] = useState(false);

  const handleToggle2FA = async () => {
    setIsToggling(true);
    const result = await toggle2FAAction();
    ...
    <>
      ...
      <div className="dash-settings-section dash-animate-fade-up dash-animate-delay-1">
        ...
        <div className="dash-settings-card">
          ...
          <div className="dash-toggle-row">
            <div className="dash-toggle-info">
              <div className="dash-toggle-info-title">OTP à chaque connexion (sécurité renforcée)</div>
              <div className="dash-toggle-info-desc">
                Demander un code email à chaque connexion, même sur les appareils connus. Recommandé si vous gérez beaucoup d&apos;argent.
              </div>
            </div>
            <button
              type="button"
              className={`dash-switch ${twoFactorEnabled ? "active" : ""}`}
